...
for i in range(10):
    ...
    print(
        f"  Test {i}: cross={'OK' if ok1 else 'FAIL'}, corners={'OK' if ok2 else 'FAIL'}, d_layer={'OK' if d_ok else 'FAIL'} -> {status}"
    )
    if status == "PASS":
        pass_count += 1
...
for _attempt in range(100):
    ...
    for edge_name in ["FR", "FL", "BR", "BL"]:
        ...
        # Determine direction
        u_target = _COLOR_TO_FACE[
            _edge_color_on_face(_find_edge(test_state, c1, c2), "U")
        ]
        ...
        print(
            f"  Edge at U above {front_face}, U-color={_edge_color_on_face(_find_edge(test_state, c1, c2), 'U')}"
        )
            ...
            for combo in product(move_set, repeat=length):
                ...
                for other_edge in ["FR", "FL", "BR", "BL"]:
                    if other_edge == edge_name:
                        continue
                    if _is_middle_edge_solved(
                        state, other_edge
                    ) and not _is_middle_edge_solved(test, other_edge):
                        other_ok = False
                        break
                if not other_ok:
                    continue
        ...
        if best:
            ...
            print("  NOT FOUND (searched up to length 9)")
