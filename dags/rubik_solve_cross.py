"""Phase 1: Solve the white cross on the D face."""
...
from include.rubik.solver import is_cross_solved, solve_cross_step


@dag(
    dag_id="rubik_solve_cross",
    schedule=None,
    max_active_runs=1,
    catchup=False,
    tags=["rubik", "solve"],
...
def rubik_solve_cross():
    ...
    def check_phase_solved(state):
        ...
        if iteration >= MAX_ITERATIONS["cross"]:
    ...
    def apply_algorithm(state):
        cube = state["cube"]
        new_cube, moves = solve_cross_step(cube)
        record_move_snapshots(state, "rubik_solve_cross", "apply_algorithm", moves, "running")
        state["cube"] = new_cube
    ...
    def prepare_next_phase(state):
        next_phase = NEXT_PHASE["cross"]
        state["phase"] = next_phase
        state["iteration"] = 0
        save_current_state(state)
        record_snapshot(state, "rubik_solve_cross", "prepare_next_phase", [], "running")
        return f"Cross solved! Moving to {next_phase}"

    @task
    def max_iterations_exceeded(state):
        reason = f"Cross phase exceeded max iterations ({MAX_ITERATIONS['cross']})"
        mark_session_failed(state, "rubik_solve_cross", "max_iterations_exceeded", reason)
    ...
    trigger_self = TriggerDagRunOperator(
        ...
        trigger_dag_id="rubik_solve_cross",
