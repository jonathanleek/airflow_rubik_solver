...
def _get_json_variable(key, default):
    value = Variable.get(key, default=None, deserialize_json=True)
    if value is None:
        return default
    return value
...
def _require_start_key(provided_key: str | None) -> None:
    expected_key = os.environ.get("RUBIK_SOLVER_START_KEY")
    if not expected_key:
        raise HTTPException(
            status_code=403,
            detail="Set RUBIK_SOLVER_START_KEY before enabling DAG triggers from the plugin.",
        )
    if provided_key != expected_key:
        raise HTTPException(status_code=403, detail="Invalid Rubik Solver start key.")
...
@app.post("/api/start")
async def start_solve(payload: dict | None = None, x_rubik_solver_key: str | None = Header(default=None)):
    ...
    _require_start_key(x_rubik_solver_key)
